...
const slices = {
  'text_with_image_slice': (slice, key) => <TextWithImageSlice key={key} slice={slice} />,
  'rich_text_slice': (slice, key) => <RichTextSlice key={key} slice={slice} />,
  'rich_text_block_slice': (slice, key) => <RichTextBlockSlice key={key} slice={slice} />,
  'cta_slice': (slice, key) => <CTASlice key={key} slice={slice} />,
  'full_width_image_slice': (slice, key) => <FullWidthImageSlice key={key} slice={slice} />,
  'hero_slice': (slice, key, page) => <HeroSlice key={key} slice={slice} page={page} />,
  'team_member_slice': (slice, key, page) => <TeamMemberSlice key={key} slice={slice} page={page} />,
  'articles': (slice, key, page) => <Articles key={key} slice={slice} page={page} />,
  'article_hero': (slice, key, page) => <ArticleHeroSlice key={key} slice={slice} page={page} />,
  'contact_us_slice': (slice, key, page) => <ContactUsSlice key={key} slice={slice} page={page} />,
  'video_slice': (slice, key, page) => <VideoSlice key={key} slice={slice} page={page} />,
  'embed_slice': (slice, key, page) => <EmbedSlice key={key} slice={slice} page={page} />
}

class Page extends PureComponent {
  addTopMargin = () => {
    const {page} = this.props
    return page.type !== 'article' && page.slices.length > 0 && first(page.slices).type !== 'hero_slice'
  }
  ...
  render () {
    ...
    forEach(page.slices, slice => {
      ...
      if (slice.type === 'color_switch_slice') {
        pageSections.push({slices: [], theme: currentPage.theme === 'dark' ? 'light' : 'dark'})
      } else if (slice.type === 'hero_slice' && slice.hero_image && slice.hero_image.images) {
        // Hero slices is always dark
        const heroComponent = slices[slice.type](slice, currentPage.slices.length, page)
        ...
        if (!slices[slice.type]) {
          ...
          currentPage.slices.push(slices[slice.type](slice, currentPage.slices.length, page))
        }
      }
    })
